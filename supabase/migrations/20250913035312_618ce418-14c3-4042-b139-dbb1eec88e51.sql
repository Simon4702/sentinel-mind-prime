...
WITH CHECK (get_user_role() IN ('admin'::user_role, 'security_analyst'::user_role));
...
CREATE POLICY "System can create behavior data" ON public.user_behaviors
...
CREATE POLICY "Users can create their own training results" ON public.training_results
FOR INSERT WITH CHECK (user_id = auth.uid());

CREATE POLICY "Security analysts can update training results" ON public.training_results
FOR UPDATE USING (get_user_role() IN ('admin'::user_role, 'security_analyst'::user_role));
